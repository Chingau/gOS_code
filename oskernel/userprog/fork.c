...
{
    /* a. 复制pcb所在的整个页，里面包含进程pcb信息及特权级0的栈，里面包含了返回地址 */
    ...
    void *vaddr_btmp = get_kernel_pages(bitmap_pg_cnt);
     /* 
      此时child_thread->userprog_vaddr.vaddr_bitmap_bits还是指向父进程虚拟地址的位图地址,
      下面将child_thread->userprog_vaddr.vaddr_bitmap_bits指向自己的位图vaddr_btmp
     */
...
static int32_t build_child_stack(struct task_struct *child_thread)
{
    ...
    /* b.为switch_to构建thread_stack_t,将其构建在紧临intr_stack之下的空间 */
    uint32_t *ret_addr_in_thread_stack = (uint32_t *)intr_0_stack - 1;
    // 下面三行不是必要的，只是为了梳理thread_stack中的关系
    ...
    *ret_addr_in_thread_stack = (uint32_t)intr_exit;
    ...
    //下面这两行赋值只是为了使构建的thread_stack更加清晰，其实也不需要，因为在进入intr_exit后一系列pop会把寄存器中的数据覆盖
...
static void update_inode_open_cnts(struct task_struct *thread)
{
    ...
    while (local_fd < MAX_FILES_OPEN_PRE_PROC) {
