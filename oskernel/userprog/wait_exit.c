...
static void release_prog_resource(struct task_struct *release_thread)
{
    ...
    uint32_t pde = 0;
    uint32_t *v_pde_ptr = NULL;     //v表示var，和函数 pde_ptr 区分
    uint16_t user_pte_nr = 1024, pte_idx = 0;
    uint32_t pte = 0;
    uint32_t *v_pte_ptr = NULL;
    uint32_t *first_pte_vaddr_in_pde = NULL;    //用来记录PDE中第1个PTE指向的物理页起始地址
    ...
    while (pde_idx < user_pde_nr) {
        ...
        if (pde & 0x00000001) {
            ...
            first_pte_vaddr_in_pde = pte_ptr(pde_idx * 0x400000);   //一个页表表示的内存容量是4MB
            pte_idx = 0;
...
/*
 list_traversal的回调函数，
 查找pelem的parent_pid是否是ppid,成功返回true，失败返回false
*/
    ...
    while (1) {
        ...
        if (child_elem != NULL) {
            ...
            *status = child_thread->exit_status;
            ...
            /*
             从就绪队列和全部队列中删除进程表项.
             传入false使thread_exit调用后回到此处，进程表项是进程或线程的最后保留的资源，至此该进程彻底消失了
            */
