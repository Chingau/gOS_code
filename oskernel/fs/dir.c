...
/* 在分区part上打开inode为inode_no的目录并返回目录指针 */
...
bool search_dir_entry(struct partition *part, struct dir *pdir, const char *name, struct dir_entry *dir_e)
{
    ...
    uint32_t *all_blocks = (uint32_t *)sys_malloc(560); //12个直接块+128个间接块共560字节
    if (all_blocks == NULL) {
    ...
    /*
     写目录项的时候已保证目录项不跨扇区
     这样读目录项的时候容易处理，只申请容纳1个扇区的内存
    */
    uint8_t *buf = (uint8_t *)sys_malloc(SECTOR_SIZE);
    struct dir_entry *p_de = (struct dir_entry *)buf;
    uint32_t dir_entry_size = part->sb->dir_entry_size;
    uint32_t dir_entry_cnt = SECTOR_SIZE / dir_entry_size;  //1个扇区内可容纳的目录项个数
    ...
    while (block_idx < block_cnt) {
        ...
        block_idx++;
        p_de = (struct dir_entry *)buf; //此时p_de已经指向扇区最后一个完整目录项，需要恢复p_de指向为buf
    ...
    while (block_idx < 140) {   //文件(包括目录)最大支持12个直接块+128个间接块=140个块
        ...
        if (all_blocks[block_idx] == 0) {
            ...
            if (block_idx < 12) {   //若是直接块
                ...
                //再分配一个块作为第0个间接块
                if (block_lba == -1) {
                    block_bitmap_idx = dir_inode->i_sectors[12] - curr_part->sb->data_start_lba;
                ...
                //每分配一步就同步一次block_bitmap
                block_bitmap_idx = block_lba - curr_part->sb->data_start_lba;
                ASSERT(block_bitmap_idx != -1);
                bitmap_sync(curr_part, block_bitmap_idx, BLOCK_BITMAP);

                all_blocks[12] = block_lba;
                //把新分配的第0个间接地址写入一级间接块表
        ...
        if (dir_entry_cnt == 1 && !is_dir_first_block) {
            ...
            if (block_idx < 12) {
                ...
                if (indirect_blocks > 1) {
                    all_blocks[block_idx] = 0;  //间接索引表中还包括其它间接块，仅在索引表中擦除当前这个间接块地址
...
struct dir_entry *dir_read(struct dir *dir)
{
    ...
    if (dir_inode->i_sectors[12] != 0) {    //若含有一级间接块表
        ide_read(curr_part->my_disk, dir_inode->i_sectors[12], all_blocks + 12, 1);
        block_cnt = 140;
    }
    block_idx = 0;
    ...
    while (dir->dir_pos < dir_inode->i_size) {
        ...
        ide_read(curr_part->my_disk, all_blocks[block_idx], dir_e, 1);
        dir_entry_idx = 0;
        //遍历扇区内所有目录项
        while (dir_entry_idx < dir_entrys_per_sec) {
            if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
...
bool dir_is_empty(struct dir *dir)
{
    ...
    return (dir_inode->i_size == (curr_part->sb->dir_entry_size * 2));
}
...
int32_t dir_remove(struct dir *parent_dir, struct dir *child_dir)
{
    ...
    delete_dir_entry(curr_part, parent_dir, child_dir_inode->i_no, io_buf);

    //回收inode中的i_sectors中所占用的扇区，并同步inode_bitmap和block_bitmap
    inode_release(curr_part, child_dir_inode->i_no);
    sys_free(io_buf);
    return 0;
}